import { createSelasClient, SelasClient, StableDiffusionConfig, PatchConfig, TrainingImage } from "../src/index";
...
describe("testing selas-node", () => {
  ...
  test("creation of job", async () => {
    ...
    const data = await selas.runStableDiffusion("banana in a kitchen",
      {patches: [PatchConfig("test-patch")]});
  ...
  test("Test the post of patch", async () => {
    ...
    let dataset = [
      TrainingImage("https://img.sanctuary.fr/fiche/origin/78.jpg",
      ...
      TrainingImage("https://ramenparados.com/wp-content/uploads/2020/10/Family-Compo-destacado.jpg",
          "fcompo style, a couple sitting on top of a red fire hydrant, a manga drawing, by Yumihiko Amano, shin hanga, city hunter, beautiful anime girl squatting, katsuhiro otomo and junji ito, realistic manga"),
      {
          ...
          "label": "fcompo style, a drawing of a woman bending over on a skateboard, a manga drawing, by Fujishima Takeji, pixiv, shin hanga, wearing a tank top and shorts, early 90s cg, ( ultra realistic ), portrait of mayuri shiina"
      },
      {
          ...
          "label": "fcompo style, a drawing of a woman holding a baseball bat, inspired by Kusumi Morikage, pixiv, shin hanga, fully clothed. painting of sexy, あかさたなは on twitter, pin on anime, initial d"
      }
    ];
    const data = await selas.runPatchTrainer(dataset, "patch-test");
    expect(data).toBeDefined();
  });
